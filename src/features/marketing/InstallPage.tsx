...
import { Smartphone, Monitor, Download, ShieldCheck, ArrowRight, Zap, Share } from 'lucide-react';
import { Link } from 'react-router-dom';

export function InstallPage() {
  useSeo({
    title: 'Install Titan - Get the PWA on Mobile & Desktop',
    description: 'Learn how to install Titan on your iPhone, Android, or Desktop. Enjoy a native-app experience with the flexibility of a PWA.',
    path: '/install-titan',
    breadcrumbs: [{ name: 'Home', item: '/' }, { name: 'Installation', item: '/install-titan' }]
  });
    ...
    <MarketingLayout>
      <section className="px-6 py-24 text-center sm:px-10">
        <div className="mx-auto max-w-4xl">
          <h1 className="mb-8 text-5xl font-extrabold tracking-tight sm:text-7xl">Titan on <span className="text-primary">Every Device.</span></h1>
          <p className="mb-12 text-lg text-muted-foreground sm:text-xl">Titan is a Progressive Web App (PWA), meaning you get a lightning-fast, native-like experience without the bloat of the App Store.</p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2">
          <div className="rounded-3xl border border-border/60 bg-card/40 p-10">
            ...
            <ol className="space-y-6 text-muted-foreground">
              <li className="flex gap-4">
                ...
                <span>Open <strong>titanapp.qzz.io</strong> in your mobile browser (Safari or Chrome).</span>
              </li>
              <li className="flex gap-4">
                ...
                <span>Tap the <strong>Share</strong> button (iOS) or the <strong>Menu</strong> icon (Android).</span>
              </li>
              <li className="flex gap-4">
          ...
          <div className="rounded-3xl border border-border/60 bg-card/40 p-10">
            ...
            <ol className="space-y-6 text-muted-foreground">
              ...
              <li className="flex gap-4">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">2</span>
                <span>Look for the <strong>Install</strong> icon (computer with an arrow) in the address bar.</span>
              </li>
              <li className="flex gap-4">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">3</span>
                <span>Click <strong>Install</strong> and confirm.</span>
              </li>
              <li className="flex gap-4">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">4</span>
      ...
      <section className="bg-primary/5 py-24">
        <div className="mx-auto max-w-4xl px-6 text-center">
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-3">
            {[
              { title: 'Offline Access', desc: 'Fully functional without an internet connection.', icon: Zap },
              { title: 'Instant Updates', desc: 'Always have the latest version without manual updates.', icon: Download },
              { title: 'Privacy', desc: 'Secure local storage for all your personal data.', icon: ShieldCheck }
            ].map((b) => (

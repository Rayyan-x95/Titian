...
export function calculateGroupBalances(groupId: string, sharedExpenses: SharedExpense[], members: string[]): GroupBalance[] {
  ...
  groupExpenses.forEach(expense => {
    ...
    expense.participants.forEach(p => { if (balances[p.id] !== undefined) balances[p.id] -= p.amount; });
  });

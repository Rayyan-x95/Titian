  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background animate-in slide-in-from-right duration-300">
      <header className="flex items-center justify-between px-6 py-4 border-b border-border/50 bg-card/50 backdrop-blur-xl sticky top-0 z-10">
        ...
        <button aria-label="Delete group" onClick={() => { void handleDeleteGroup(); }} className="flex h-10 w-10 items-center justify-center rounded-full text-muted-foreground hover:bg-rose-500/10 hover:text-rose-500 transition-colors"><Trash2 className="h-5 w-5" /></button>
      </header>
        ...
        <div className="px-6 mt-8">
          {activeTab === 'balances' ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3">{groupExpenses.length === 0 ? (<div className="text-center py-12 text-muted-foreground"><Receipt className="h-10 w-10 mx-auto opacity-20 mb-3" /><p className="text-xs font-bold uppercase tracking-widest">No expenses yet</p></div>) : (groupExpenses.map(expense => { const payer = groupMembers.find(m => m.id === expense.paidBy)?.name || 'Unknown'; return (<div key={expense.id} className="group relative flex items-center justify-between rounded-2xl border border-border/50 bg-card/30 p-4 hover:border-primary/30 transition-colors"><div className="flex items-center gap-3"><div className={cn("flex h-10 w-10 items-center justify-center rounded-xl", expense.isSettlement ? "bg-emerald-500/10 text-emerald-500" : "bg-primary/10 text-primary")}><Receipt className="h-5 w-5" /></div><div><div className="flex items-center gap-2"><p className="text-sm font-bold">{expense.description}</p>{expense.isSettlement && <span className="rounded-full bg-emerald-500/10 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider text-emerald-500 border border-emerald-500/20">Settlement</span>}</div><p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wide">Paid by {payer}</p></div></div><div className="text-right"><p className="text-sm font-bold">{formatMoney(expense.totalAmount, currency)}</p><button onClick={() => { void deleteSharedExpense(expense.id); }} className="mt-1 text-[10px] font-bold text-rose-500/50 hover:text-rose-500 uppercase tracking-widest transition-colors opacity-0 group-hover:opacity-100">Remove</button></div></div>); }))}</div>
            </div>
          )}
        </div>
      </div>

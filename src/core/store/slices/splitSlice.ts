...
import type { Friend, FriendInput, FriendUpdate, Group, GroupInput, GroupUpdate, SharedExpense, SharedExpenseInput } from '../types';
...
export const createSplitSlice: StateCreator<CoreStoreState, [], [], SplitSlice> = (set, get) => ({
  ...
  deleteFriend: async (id) => {
    ...
    const nextSharedExpenses = currentState.sharedExpenses.map(se => {
      ...
      if (nextParticipants.length === 0 && nextPaidBy !== 'user') {
        console.warn(`[Titan] Shared expense ${se.id} would have no participants; setting paidBy to 'user'`);
        nextPaidBy = 'user';
        modified = true;
      }
  ...
  addGroup: async (input) => {
    ...
    if (missingFriends.length > 0) {
      throw new Error(`Cannot create group: Friends with IDs ${missingFriends.join(', ')} do not exist.`);
    }
  ...
  addSharedExpense: async (input) => {
    ...
    if (missingParticipants.length > 0) {
      throw new Error(`Cannot add shared expense: Participants ${missingParticipants.map(mp => mp.id).join(', ')} do not exist.`);
    }

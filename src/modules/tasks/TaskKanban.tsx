...
export function TaskKanban({ tasks, onEditTask }: TaskKanbanProps) {
  ...
  const columnsData = useMemo(() => {
    ...
    // Only top level tasks in kanban to keep it clean
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (newStatus !== oldStatus) {
      ...
      // Update snapshot if completed/uncompleted
      if (newStatus === 'done') {
                          ...
                          <div className="flex flex-wrap gap-2 items-center mt-3">
                            {task.dueDate && (
                              <span className={cn(
                                ...
                                new Date(task.dueDate) < new Date() && task.status !== 'done' ? "bg-rose-500/10 text-rose-500" : "bg-secondary text-muted-foreground"
                              )}>
                            ...
                            {(task.area || task.tags?.length) && (
                              <div className="flex items-center gap-1 overflow-hidden">
                                {task.area && <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">{task.area}</span>}
                                {task.tags?.slice(0, 2).map(tag => (
                                  <span key={tag} className="text-[9px] font-bold text-primary/70 uppercase tracking-widest before:content-['•'] before:mr-1 before:text-muted-foreground/30">
                                    {tag}
                                  </span>
                                ))}
                              </div>
